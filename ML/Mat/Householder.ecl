  ...
  EXPORT HTA := MODULE
	   EXPORT Default := MODULE,VIRTUAL
		 ...
		 EXPORT Atkinson := MODULE(Default)
				...
				EXPORT DATASET(ML_Mat.Types.Element) hReflection := Sub(IdentityM, Scale(Mul(houseVec,Trans(houseVec)),2));
		 END;
		 ...
		 EXPORT Golub := MODULE(Default)
				...
				EXPORT DATASET(ML_Mat.Types.Element) hReflection := Sub(IdentityM, Scale(Mul(hV,Trans(hV)),Beta));
		 END;
