...
EXPORT Sampling := MODULE
	...
	SHARED idFoldRec := RECORD
		 ...
		 ML.Types.t_RecordID id;
	END;
	SHARED  ML.Types.NumericField CreateIdRecs( ML.Types.NumericField L, INTEGER C, INTEGER origSize, ML.Types.t_Discrete method = g_Method.Zeroing) := TRANSFORM
		SELF.id := c;
		SELF.number := 0;
		SELF.value := IF(method = g_Method.Zeroing, 0, RANDOM()%origSize + 1);
	END;	
	SHARED GenerateIdList(Types.t_Count origSize, ML.Types.t_Discrete pctSize = 100,  ML.Types.t_RecordID baseId = 0) := FUNCTION
		RETURN PROJECT(Vec.From(origSize * pctSize DIV 100),TRANSFORM(idListRec, SELF.id:=LEFT.x + baseId, SELF.oldid := RANDOM()%origSize + 1));
	END;
	SHARED dsRecordRnd := RECORD( ML.Types.NumericField)
		 ML.Types.t_FieldNumber rnd:= 0;	
	END;
	SHARED dsRecordRnd AddRandom( ML.Types.NumericField l) :=TRANSFORM
		SELF.rnd := RANDOM();
		SELF := l;
	END;
	SHARED  ML.Types.NumericField GetRecords( ML.Types.NumericField l, idListRec r) := TRANSFORM
		SELF.id := r.id;
		SELF.number := l.number;
		SELF.value := l.value;
	END;
	SHARED dsDiscRecRnd := RECORD(ML.Types.DiscreteField)
		 ML.Types.t_FieldNumber rnd:= 0;	
	END;
...
// In order to get the complete dataset JOIN with original dataset is needed.
	EXPORT GenerateNSampleList(t_Index N,  ML.Types.t_RecordID origSize) := FUNCTION
...
//
	EXPORT GeneratedData(DATASET(idListRec) dsIdList, DATASET( ML.Types.NumericField) origData) := MODULE
...
//
	EXPORT RndSampleWithoutReplace(DATASET( ML.Types.NumericField) originalData, ML.Types.t_Discrete pctSize = 100,  ML.Types.t_RecordID baseId = 0) := FUNCTION
...
//
	EXPORT RndSampleWithReplace(DATASET( ML.Types.NumericField) originalData, ML.Types.t_Discrete pctSize = 100,  ML.Types.t_RecordID baseId = 0) := FUNCTION
		numIdx 	:= MIN(originalData, number);
		dsIds 	:= originalData(number = numIdx);
		origSize := COUNT(dsIds);
...
//
	EXPORT StratSampleWithReplace(DATASET( ML.Types.NumericField) originalData, ML.Types.t_Discrete pctSize = 100,  ML.Types.t_RecordID baseId = 0,  ML.Types.t_FieldNumber fieldClass = 0) := FUNCTION
		...
		dsGenId		:= NORMALIZE(classDist, LEFT.pSize, TRANSFORM( ML.Types.NumericField, SELF.id:= 1 + RANDOM()%LEFT.Support, SELF.number:= COUNTER + LEFT.Offset, SELF:=LEFT));
...
//
	EXPORT DiscreteDepNFolds(DATASET(ML.Types.DiscreteField) originalData,  ML.Types.t_FieldNumber num_part) := FUNCTION
		dRnd := PROJECT(originalData, TRANSFORM(dsDiscRecRnd, SELF.rnd := RANDOM(), SELF:= LEFT));
		dRndSorted := SORT(dRnd, value, rnd);
		ds_part := PROJECT(dRndSorted, TRANSFORM(ML.Types.DiscreteField, SELF.number := COUNTER%num_part + 1, SELF:= LEFT));
...
//
	EXPORT NFold(DATASET( ML.Types.NumericField) originalData, ML.Types.t_Discrete num_part,  ML.Types.t_FieldNumber fieldDistrib = 0) := MODULE
		...
		EXPORT FoldNDS(ML.Types.t_Discrete num_fold,  ML.Types.t_RecordID baseId = 0) := FUNCTION
			ds_ids := ds_parts(fold = num_fold);
			RETURN JOIN(originalData, ds_ids,LEFT.id = RIGHT.id, TRANSFORM( ML.Types.NumericField, SELF.id:= LEFT.id + baseId, SELF:=LEFT), LOOKUP); 			
		END;
	END;
...
//		fieldDistrib	: [OPTIONAL] field having distribution desired, default 0 indicate last field
//
	EXPORT NFoldCross(DATASET( ML.Types.NumericField) originalData, ML.Types.t_Discrete num_part,  ML.Types.t_FieldNumber fieldDistrib = 0) := MODULE
		...
		EXPORT FoldNTrainDS (ML.Types.t_Discrete num_fold,  ML.Types.t_RecordID baseId = 0) := FUNCTION
			ds_ids := ds_parts(fold!= num_fold);
			RETURN JOIN(originalData, ds_ids,LEFT.id = RIGHT.id, TRANSFORM( ML.Types.NumericField, SELF.id:= LEFT.id + baseId, SELF:=LEFT), LOOKUP); 
		END;
		EXPORT FoldNTestDS (ML.Types.t_Discrete num_fold,  ML.Types.t_RecordID baseId = 0) := FUNCTION
			ds_ids := ds_parts(fold = num_fold);
			RETURN JOIN(originalData, ds_ids,LEFT.id = RIGHT.id, TRANSFORM( ML.Types.NumericField, SELF.id:= LEFT.id + baseId, SELF:=LEFT), LOOKUP); 
		END;
	END;
